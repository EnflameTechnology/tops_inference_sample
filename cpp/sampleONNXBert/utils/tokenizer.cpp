...
map<std::string, int> read_vocab(const char *filename) {
  ...
  while (getline(fs8, line)) {
    ...
    vocab.insert(
        pair<std::string, int>(std::string(line.begin(), line.end()), index));
    index++;
    line_count++;
  }
  return vocab;
...
std::string BasicTokenizer::_run_strip_accents(std::string text) {
  ...
  int i = 0;
  while (i < text_.size()) {
    long int cat = unicode::category(int(text_[i]));
    if (cat != categories["Mn"]) {
      output.push_back(text_[i]);
    }
...
vector<int> BertTokenizer::tokens_to_ids(vector<std::string> tokens) {
  vector<int> ids;
  vector<std::string>::iterator ptr;
  for (ptr = tokens.begin(); ptr < tokens.end(); ptr++) {
    ids.push_back(vocab[*ptr]);
  }
  return ids;
}

void BertTokenizer::encode(std::string textA, std::string textB,
                           vector<float> &input_ids, vector<float> &input_mask,
                           vector<float> &segment_ids, int max_seq_length,
  ...
  if (textB == "") {
    ...
    basictokenizer.truncate_sequences(tokens_A, tokens_B, truncation_strategy,
                                      max_seq_length - 3);
    // insert "[CLS}"
    tokens_A.insert(tokens_A.begin(), "[CLS]");
    // insert "[SEP]"
