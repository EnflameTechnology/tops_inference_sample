...
bool nonMaximumSuppression(const std::vector<Rect> rects,
        ...
        if (!is_suppressed[indices[i]]) {
            for (j = i + 1; j < num_boxes; j++) {
                if (!is_suppressed[indices[j]]) {
                    int x1max = std::max(rects[indices[i]].left,
                                         rects[indices[j]].left);
                    int x2min = std::min(
                        rects[indices[i]].left + rects[indices[i]].width,
                        rects[indices[j]].left + rects[indices[j]].width);
                    int y1max =
                        std::max(rects[indices[i]].top, rects[indices[j]].top);
                    int y2min = std::min(
                        rects[indices[i]].top + rects[indices[i]].height,
    ...
    for (i = 0; i < num_boxes; i++) {
        if (!is_suppressed[i]) index_out.push_back(i);
    }
...
std::vector<Rect> postProcess(std::vector<float> &pred, const float img_height,
    ...
    for (int id : indexes) {
        ...
        std::cout << "cls: " << label_map[class_id[id]] << " conf: " << confidence[id]
                  << " (" << selected_boxes[id].left << ","
                  << selected_boxes[id].top << "," << selected_boxes[id].width
                  << "," << selected_boxes[id].height << ")" << std::endl;
    }
...
void writePPMFileWithBBox(const std::string &filename, uint8_t *ppm,
    ...
    for (auto &bbox : bboxs) {
        ...
        const int x2 = std::min(std::max(0, round(int(bbox.left + bbox.width))),
                                ppm_w - 1);
        const int y1 = std::min(std::max(0, round(int(bbox.top))), ppm_h - 1);
        const int y2 = std::min(std::max(0, round(int(bbox.top + bbox.height))),
                                ppm_h - 1);
...
int main(int argc, char **argv) {
    ...
    TopsInference::IEngine *engine =
        loadOrCreateEngine(exec_path.c_str(), onnx_path,
    ...
    // 8: Drawing and save
    writePPMFileWithBBox("./result.ppm",
                         static_cast<uint8_t *>(ppm_data.data()), result, ppm_h,
                         ppm_w, ppm_max);
